...
from C02_emissions_alternative import calculate_schedule_emissions, getCO2_em
...
powerMultiplicator = 800 
data['energy_need'] = data['energy_need'] * powerMultiplicator

load = data['fixedLoad'] #load curve for 05/06/2019
arrival = 36
departure = 13
energy_need = data['energy_need'][0]
max_power = energy_need/3
...
base_emissions = calculate_schedule_emissions(load)
print(f"Water-filling: Emissions = {water_filling_emissions - base_emissions:.4f} TCO2")
print(f"Linear Programming: Emissions = {linprog_emissions - base_emissions:.4f} TCO2")
print(f"MILP: Emissions = {MILP_emissions - base_emissions:.4f} TCO2")
...
ax.plot(hours, load, 'k-', linewidth=2, label='Fixed Load')
ax.plot(hours, load + water_filling_schedule, 'o-', label='Water-Filling', color='blue', alpha=0.7)
ax.plot(hours, load + linprog_result.x, 's-', label='Linear Programming', color='orange', alpha=0.7)
ax.plot(hours, load + MILP_result, '^-', label='MILP', color='red', alpha=0.7)

#plot departure and arrival times
ax.axvline(x=arrival, linestyle='--', label='Arrival Time', color='green')
ax.axvline(x=departure, linestyle='--', label='Departure Time', color='red')


ax.add_patch(plt.Rectangle((-100, 40100), len(load)+200, 4230, alpha=0.1, color='brown'))
ax.add_patch(plt.Rectangle((-100, 0), len(load)+200, 40100, alpha=0.1, color='green'))
ax.add_patch(plt.Rectangle((-100, 44230), len(load)+200, 21370, alpha=0.1, color='yellow'))

ax.text(len(load)/2, 40800, f'Biomass production range({getCO2_em()["bioen"]}gCO2/kwh)', ha='center', va='center', fontsize=10)
ax.text(len(load)/2, 35700, f'Renewable production range ({getCO2_em()["hydro"]}gCO2/kwh)', ha='center', va='center', fontsize=10)
ax.text(len(load)/2, 45030, f'Nuclear production range ({getCO2_em()["nuclear"]}gCO2/kwh)', ha='center', va='center', fontsize=10)


ax.set_xlabel('time (1/2h)')
ax.set_ylabel('Power [MW]')
ax.set_title('EV Charging Schedules Comparison')
ax.legend()
